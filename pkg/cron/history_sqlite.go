...
// NewHistoryStore opens (or creates) a SQLite database at dbPath and returns
// a HistoryStore backed by it.
...
func (hs *sqliteHistoryStore) Close() error {
	return hs.db.Close()
}

func (hs *sqliteHistoryStore) WriteRecord(rec ExecutionRecord) error {
...
func (hs *sqliteHistoryStore) OnExecutionComplete(rec ExecutionRecord) {
	hs.WriteRecord(rec) //nolint:errcheck
}

func (hs *sqliteHistoryStore) QueryHistory(jobID string, page, size int) ([]ExecutionRecord, int, error) {
	if page < 1 {
		page = 1
	}
	if size < 1 || size > 100 {
...
func (hs *sqliteHistoryStore) Stats24h(nowMS int64) (HistoryStats, error) {
...
func (hs *sqliteHistoryStore) Trend(nowMS int64, days int) ([]TrendEntry, error) {
	if days < 1 {
		days = 7
	}
